...
# Import modules that use the mcp variable (must come after mcp is created)
from .fpl.resources import fixtures, gameweeks, players, teams  # noqa: E402
from .fpl.tools import (  # noqa: E402
...
async def get_player_fixtures_by_name(player_name: str) -> dict[str, Any]:
    ...
    return {
        "player": {"name": player["name"], "team": player["team"], "position": player["position"]},
...
def player_analysis_prompt(player_name: str, include_comparisons: bool = True) -> str:
    ...
    """
    comparison_text = (
        "\n5. How they compare to similar players in their position and price range" if include_comparisons else ""
    )
