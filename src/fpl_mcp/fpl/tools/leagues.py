...
async def get_league_standings_data(league_id: int) -> dict[str, Any]:
    ...
    try:
        ...
        return {"error": f"Failed to retrieve league standings: {str(e)}"}
...
async def get_teams_historical_data(
    team_ids: list[int], start_gw: int | None = None, end_gw: int | None = None
) -> dict[str, Any]:
    """
...
async def _get_league_historical_performance(
    league_id: int, start_gw: int | None = None, end_gw: int | None = None
) -> dict[str, Any]:
    """
    ...
    for gw_index, gw in enumerate(gameweeks):
        ...
        for team in series:
            ...
            if points > max_points:
                max_points = points
                winner = {"team_id": team["team_id"], "name": team["name"], "points": points}
...
async def _get_league_team_composition(league_id: int, gameweek: int | None = None) -> dict[str, Any]:
    """
    ...
    if not teams_data:
        return {"error": "Failed to retrieve team data for any teams in the league", "errors": errors}
    ...
    players_by_ownership = sorted(
        player_ownership.values(), key=lambda p: (p["ownership_percent"], p["total_points"]), reverse=True
    )
...
async def _get_league_fixture_analysis(
    ...
    gameweek_fixtures = [f for f in all_fixtures if f.get("event") and start_gw <= f.get("event") <= end_gw]
    ...
    picks_results = await gather_limited(
        (auth_manager.get_team_for_gameweek(t["team_id"], start_gw) for t in top_teams),
        limit=5,
        return_exceptions=True,
    )
    picks_by_team = {}
    for fantasy_team, picks in zip(top_teams, picks_results):
        if isinstance(picks, Exception):
            logger.error(f"Error fetching team {fantasy_team['team_id']} for gameweek {start_gw}: {picks}")
            continue
        picks_by_team[fantasy_team["team_id"]] = picks

    # Get double and blank gameweeks in the specified range (needed inside
    # the per-team loop for blank_gameweek_impact and in the final result)
    try:
    ...
    for rank, fantasy_team in enumerate(top_teams):
        ...
        try:
            ...
            player_teams = {
                player_id: player_team_map[player_id] for player_id in player_ids if player_id in player_team_map
            }
            ...
            for gw in gameweeks:
                gameweek_difficulty[gw] = {"total_players": 0, "total_difficulty": 0, "teams_with_fixtures": 0}
            ...
            top_teams = sorted(
                [(fpl_id, count) for fpl_id, count in fpl_team_counts.items()], key=lambda x: x[1], reverse=True
            )[:3]
            ...
            team_fixture_analysis.append(
                {
                    ...
                    "blank_gameweek_impact": [
                        ...
                        for bw in blank_gameweeks
                        if any(t["id"] in fpl_team_counts for t in bw["teams_without_fixtures"])
                    ],
                }
            )
    ...
    return {
        "league_info": league_data["league_info"],
        "gameweek_range": {"start": start_gw, "end": end_gw, "gameweeks": gameweeks},
        "team_fixture_analysis": team_fixture_analysis,
        "special_gameweeks": {"blank_gameweeks": blank_gameweeks, "double_gameweeks": double_gameweeks},
    }


async def _get_league_analytics(
    league_id: int, analysis_type: str = "overview", start_gw: int | None = None, end_gw: int | None = None
) -> dict[str, Any]:
    """
    ...
    try:
        if analysis_type == "overview" or analysis_type == "historical":
            ...
            return await _get_league_historical_performance(league_id, effective_start_gw, effective_end_gw)
            ...
            return await _get_league_team_composition(league_id, effective_end_gw)
            ...
            return await _get_league_fixture_analysis(league_id, effective_start_gw, effective_end_gw)
    except Exception as e:
